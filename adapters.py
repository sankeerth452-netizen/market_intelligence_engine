...
the generic crawler.py (text) + realworld.py (the semantic_gap signal). Keeping
them apart is deliberate: the crawler is per-client config; these adapters are not.
...
Sources, by setup cost:
  ...
  * trend_surprise / trend_changepoint -> Google Trends (pytrends)  [next]
...
if __name__ == "__main__":   # quick probe against the live feed
